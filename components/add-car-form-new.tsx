        ...
        <ScrollArea className="max-h-[calc(85vh-100px)]">
          <form onSubmit={handleSubmit} className="space-y-5 px-6 pb-6">
            ...
            {isPartnership && hasPartners && (
              <div className="space-y-3 p-4 bg-secondary/50 rounded-lg">
                <Label>{t('label.selectPartners', language)}</Label>
                {partners.map((partner) => (
                  <div key={partner.id} className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      id={`partner-${partner.id}`}
                      checked={shares[partner.id] !== undefined}
                      onChange={(e) => {
                        const isChecked = e.target.checked
                        setShares(prev => {
                          const newShares = { ...prev }
                          if (isChecked) {
                            // Add this partner with equal share
                            const activePartners = Object.keys(newShares).filter(id => newShares[id] !== undefined)
                            const equalShare = Math.floor(100 / activePartners.length)
                            const remainder = 100 - (equalShare * activePartners.length)
                            
                            activePartners.forEach((id, index) => {
                              const currentShare = newShares[id] || 0
                              if (id === partner.id) {
                                newShares[id] = equalShare + (index === 0 ? remainder : 0)
                              } else {
                                newShares[id] = currentShare
                              }
                            })
                          } else {
                            // Remove this partner
                            delete newShares[partner.id]
                            
                            // Redistribute shares among remaining partners
                            const remainingPartners = Object.keys(newShares).filter(id => newShares[id] !== undefined)
                            if (remainingPartners.length > 0) {
                              const newEqualShare = Math.floor(100 / remainingPartners.length)
                              const newRemainder = 100 - (newEqualShare * remainingPartners.length)
                              
                              remainingPartners.forEach((id, index) => {
                                newShares[id] = newEqualShare + (index === 0 ? newRemainder : 0)
                              })
                            }
                          }
                          return newShares
                      ...
                      className="w-4 h-4"
                    />
                    <Label htmlFor={`partner-${partner.id}`} className="flex-1 text-sm">
                      {partner.name}
                    </Label>
                    <Input
                      type="number"
                      ...
                      min="0"
                      max="100"
                      disabled={shares[partner.id] === undefined}
                    />
                ...
                {totalShares !== 100 && totalShares > 0 && (
                  <p className="text-sm text-destructive">
                    {t('label.totalShares', language)}: {totalShares}% ({t('warning.shouldBe100', language)})
                  </p>
                )}
              </div>
            )}
