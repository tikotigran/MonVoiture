...
class RealAdminService {
  ...
  async getCars(): Promise<AdminCar[]> {
    ...
    try {
      ...
      console.log('Admin cars loaded successfully:', cars.length)
  ...
  async getPopularBrands(): Promise<PopularBrand[]> {
    ...
    try {
      ...
      console.log('Admin brands loaded successfully:', Object.keys(brandCounts).length)
  ...
  async getCategoryExpenses(): Promise<CategoryExpense[]> {
    ...
    try {
      ...
      console.log('Admin categories loaded successfully:', Object.keys(categoryTotals).length)
  ...
  async sendNotification(title: string, message: string, onlyActive: boolean = false): Promise<void> {
    ...
    console.log('Admin sending notification:', { title, message, onlyActive })
    // В реальном приложении здесь будет логика отправки через FCM
  }
  ...
  async cleanupInactiveUsers(): Promise<number> {
    ...
    console.log('Admin cleaning up inactive users')
    // В реальном приложении здесь будет логика очистки
    return 1
  }
}

export const realAdminService = new RealAdminService()
